...
def plot_3d_bar_chart(df, custom_colors):
    ...
    # 绘制3D柱状图（无边框）
    ax.bar3d(xpos, ypos, zpos, dx, dy, dz, color=colors, alpha=0.56, edgecolor='none', linewidth=0)
...
if __name__ == "__main__":
    ...
    ax.set_yticklabels(policy_display_names, fontsize=7, fontproperties=times_font)
    ...
    # 保存图片
    output_png = os.path.join(script_dir, '3d_bar_W_solid_output.png')
    output_svg = os.path.join(script_dir, '3d_bar_W_solid_output.svg')

    plt.savefig(output_svg, format='svg', dpi=300, bbox_inches='tight')
    print(f"W indicator 3D Bar Chart (Solid) saved as '{output_svg}'")
