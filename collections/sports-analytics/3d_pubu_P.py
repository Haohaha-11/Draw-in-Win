...
for i, policy in enumerate(reversed(list(policies_display))):
    ...
    for j, (x, y, z) in enumerate(zip(x_valid, y_points, z_valid)):
        ...
        offset = 0.015 if j % 2 == 0 else 0.02
        text_color = text_colors[policy]
        ax.text(x, y, z + offset, label, ha='center', va='bottom',
                fontsize=9, fontproperties=times_font, color=text_color, fontweight='bold')
...
# Save outputs
svg_path = os.path.join(script_dir, '3d_waterfall_P.svg')
png_path = os.path.join(script_dir, '3d_waterfall_P.png')

plt.savefig(svg_path, format='svg', dpi=300, bbox_inches='tight')
print(f"P indicator 3D Waterfall Chart saved as '{svg_path}'")
