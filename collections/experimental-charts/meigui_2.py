...
def draw_final_chart(ax, chart_data):
    ...
    # 1. 绘制内圈底座
    ax.bar(x=thetas, height=inner_heights, width=widths, color=inner_colors,
           align='edge', edgecolor='white', linewidth=1.5)
    # 2. 绘制外圈数据环
    ax.bar(x=thetas, height=[colored_ring_width] * num_vars, width=widths,
    ...
    for i in range(num_vars):
        ...
        ax.text(label_angle_rad, label_radius, f'{values[i]:.1f}%',
    ...
    legend_handles = [mpatches.Patch(color=colors[i], label=legend_labels[i]) for i in range(num_vars)]
    # 在子图上显示图例
    ax.legend(handles=legend_handles, loc='center left',
              bbox_to_anchor=(0.85, 0.5),
              frameon=False, fontsize=12, labelspacing=0.05)
...
# 定义所有图表共享的颜色和图例标签
common_colors = [
    '#C42121', '#E84D5B', '#F27676', '#FF9980', '#FFB479',
    '#FFE083', '#FFFF99', '#ACD6FF', '#79ABDC', '#4682B4',
    '#9AD4D4', '#60B4B4', '#008B8B', '#7D7DFF', '#6A5ACD']
# 颜色顺序和图例标签顺序需要和您的数据值顺序一一对应
common_legend_labels = [
    'A1-8', 'B2-10', 'B1-7', 'C1-1', 'A1-1', 'A1-3', 'A1-5', 'B2-6',
    'B2-15', 'C1-6', 'A3-1', 'B2-8', 'A1-11', 'A1-2', 'A1-3']
...
if num_charts > 0:
    ...
    fig, axes = plt.subplots(nrows, ncols, figsize=(ncols * 5, nrows * 5), subplot_kw={'projection': 'polar'})
