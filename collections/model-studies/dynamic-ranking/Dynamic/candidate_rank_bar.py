...
df = pd.read_csv(os.path.join(script_dir, 'baseline_candidate_rank.csv'))
...
# 打印baseline选手和颜色对应关系
print("Baseline选手颜色分配：")
for i, player in enumerate(df_sorted['player_id']):
    if i < len(color_palette):
        print(f"  排名{i+1} - {player}: {color_palette[i]}")
...
def plot_bar_chart(save_path=None):
    ...
    # 打印Baseline实际绘图顺序和颜色
    print("\nBaseline实际绘图顺序和颜色：")
    for i, (player, score) in enumerate(zip(players, total_scores)):
        if i < len(color_palette):
            print(f"  排名{i+1} - {player}: {color_palette[i]} (分数: {score:.2f})")

    # 创建图表
    fig, ax = plt.subplots(figsize=(10, 7))

    # 为每个选手分配颜色（按排名顺序）
    colors = [color_palette[i] if i < len(color_palette) else '#cccccc' for i in range(len(players))]
    ...
    # 添加标题
    ax.set_title('Baseline Candidate Players Ranking', fontsize=16, fontweight='bold', pad=15)
...
fig, ax = plot_bar_chart(os.path.join(output_dir, 'baseline_candidate_rank_bar.png'))
plt.savefig(os.path.join(output_dir, 'baseline_candidate_rank_bar.pdf'), bbox_inches='tight')
plt.savefig(os.path.join(output_dir, 'baseline_candidate_rank_bar.svg'), bbox_inches='tight')
plt.show()

print("所有格式的候选球员排名条形图已生成并保存！")
print(f"排名前三的球员：")
