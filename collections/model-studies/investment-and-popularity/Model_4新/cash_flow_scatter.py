...
def plot_cash_flow_scatter(data_folder, save_path):
    """
    绘制投资-现金流散点图，点的大小表示投资强度
    ...
    for filename, label in files.items():
        ...
        c_values = df_sampled['C'].values  # 使用C（现金流）而不是Pop
        ...
        # 绘制散点图（去掉边框）
        plt.scatter(t_values, c_values, s=sizes, c=colors[label],
        ...
        if len(t_values) > 3:  # 需要至少4个点才能做三次样条插值
            ...
            # 使用三次样条插值
            spl = make_interp_spline(t_values, c_values, k=3)
            c_smooth = spl(t_smooth)

            # 绘制平滑曲线
            plt.plot(t_smooth, c_smooth, color=colors[label],
                    linewidth=2, alpha=0.6, zorder=2)

    # 设置图表属性
    plt.xlabel('Time ($t$)', fontsize=22)
    plt.ylabel('Cash Flow ($C$)', fontsize=22)
...
if __name__ == '__main__':
    # 设置路径
    data_folder = '.'
    save_path = 'cash_flow_scatter.png'

    # 绘制图表
    plot_cash_flow_scatter(data_folder, save_path)

    # 同时保存PDF和SVG格式
    save_path_pdf = 'cash_flow_scatter.pdf'
    save_path_svg = 'cash_flow_scatter.svg'

    plot_cash_flow_scatter(data_folder, save_path_pdf)
    plot_cash_flow_scatter(data_folder, save_path_svg)
